import CircularProgress from "@mui/material/CircularProgress";
import Typography from "@mui/material/Typography";
import Box from "@mui/material/Box";
import PropTypes from "prop-types";
import { useId } from "react";
import { useTheme } from "@mui/material/styles";

function CircularProgressWithLabel({ value }) {
  const theme = useTheme();
  const mode = theme.palette.mode;
  const gradientId = useId();

  const getStyles = (val) => {
    if (val === Infinity || isNaN(val) || val <= 30) {
      return {
        gradientColors: theme.colors.gradients.high.colors[mode],
        backgroundColor: theme.colors.gradients.high.background,
        typographyGradient: theme.colors.gradients.high.typographyGradient,
      };
    } else if (val <= 70) {
      return {
        gradientColors: theme.colors.gradients.medium.colors[mode],
        backgroundColor: theme.colors.gradients.medium.background,
        typographyGradient: theme.colors.gradients.medium.typographyGradient,
      };
    } else {
      return {
        gradientColors: theme.colors.gradients.low.colors[mode],
        backgroundColor: theme.colors.gradients.low.background,
        typographyGradient: theme.colors.gradients.low.typographyGradient,
      };
    }
  };

  const createGradient = (colors) => (
    <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
      {colors.map((color, index) => (
        <stop
          key={index}
          offset={`${index * 100}%`}
          style={{ stopColor: color }}
        />
      ))}
    </linearGradient>
  );

  const safeValue = value === Infinity || isNaN(value)
    ? Infinity
    : Math.max(0, Math.min(value, 100));

  const { gradientColors, backgroundColor, typographyGradient } = getStyles(safeValue);

  return (
    <Box sx={{ position: "relative", display: "inline-flex" }}>
      {/* Foreground ring with gradient */}
      <CircularProgress
        variant="determinate"
        value={safeValue === Infinity ? 100 : safeValue}
        size={85}
        thickness={5}
        sx={{
          color: "transparent",
          zIndex: 1,
          "& .MuiCircularProgress-circle": {
            strokeLinecap: "round",
            stroke: `url(#${gradientId})`,
          },
        }}
      />

      {/* Background ring */}
      <CircularProgress
        variant="determinate"
        value={100}
        size={85}
        thickness={5}
        sx={{
          position: "absolute",
          top: 0,
          left: 0,
          color: backgroundColor,
          zIndex: 0,
        }}
      />

      {/* Percentage label */}
      <Box
        sx={{
          position: "absolute",
          inset: 0,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
        }}
      >
        <Typography
          variant="caption"
          fontSize="larger"
          component="div"
          sx={{
            background: typographyGradient,
            backgroundClip: "text",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
            fontWeight: 600,
          }}
        >
          {safeValue === Infinity ? "∞" : `${Math.round(safeValue)}%`}
        </Typography>
      </Box>

      {/* Inject gradient into DOM */}
      <svg width="0" height="0">
        <defs>{createGradient(gradientColors)}</defs>
      </svg>
    </Box>
  );
}

CircularProgressWithLabel.propTypes = {
  value: PropTypes.number.isRequired,
};

export default CircularProgressWithLabel;
